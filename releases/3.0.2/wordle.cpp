...
void pause(){
	...
	while(1){
		...
		if(EM.lbutton){
			if(EM.y>=cursory-2&&EM.y<=cursory+18&&EM.x>=10&&EM.x<=250)
				break;
		}
	}
}
string user;
int back=RGB(10,10,10);
int red=RGB(250,10,10);
int green=RGB(10,250,10);
int blue=RGB(10,10,250);
int yellow=RGB(250,250,10);
int purple=RGB(250,10,250);
void scta(int clr){
	settextcolor(clr);
}
	...
	while(1){
		...
		while(1){
			...
			if(EM.message==WM_CHAR){
				...
				if(EM.y>=BG-2&&EM.y<=ED-2&&EM.x>=10&&EM.x<=250){
					t=(EM.y-BG+22)/20;
				}
				else continue;
			}
		...
		while(1){
			...
			if(t==1){
				...
				if(vals.count(S)){
					...
					if(tmp=="") break;
					bool flg=true;
					for(int i=0;i<(int)(tmp.size());i++) if(!isdigit(tmp[i])){
						flg=false;
						break;
					}
					if(!flg){
						putstring("Please enter a positive integer!");
						changeline();
						pause();
						break;
					}
...
void username(){
	clear_screen();
	putstring("Change Username");changeline();
