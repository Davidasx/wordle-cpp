...
void clr(int R){
	if(R<=500) scta(back);
	else if(R<=800) scta(green);
	else if(R<=1000) scta(blue);
	else if(R<=1300) scta(purple);
	else if(R<=1700) scta(yellow);
	else if(R<=2000) scta(red);
}
...
int download_file(string pth,string out){
	string url="https://gh.api.99988866.xyz/https://raw.githubusercontent.com/Davidasx/wordle-cpp/main/"+pth;
...
int main(int argc,char** argv){
	...
	if(time(NULL)>vals["LAST_UPDATE"]+vals["UPDATE_INTERVAL"]*24*3600){
		scta(red);
		cout<<"WARNING:This version of wordle is old."<<endl;
		cout<<"       Please update as soon as possible."<<endl;
		scta(back);
		vals["LAST_UPDATE"]=time(NULL);
		system("pause");
		system("cls");
	}
	scta(back);
